...
export class JwtService {
  ...
  async createJwt(userId: string, deviceId: string): Promise<TokensType> {
    ...
    const accessToken = jwt.sign({ userId: userId }, secret.ACCESS_TOKEN_SECRET, { expiresIn: "7h" });
    const refreshToken = jwt.sign({ userId: userId, deviceId: deviceId }, secret.REFRESH_TOKEN_SECRET, { expiresIn: "7h" });
